        ...
        <div className='p-5'>
          ...
          <div className='flex items-center justify-between mt-8'>
            <div className='flex gap-3'>
              {tags.map(tag => (
                <img className='rounded-lg size-10 hover-animation' key={tag.id} src={tag.path} alt={tag.name} />
              ))}
            </div>
            <a className='inline-flex items-center gap-4 font-medium hover-animation cursor-pointer hover-animation' href={href}>
              View Project
              <img src='assets/arrow-up.svg' alt='View Project Icon' />
            </a>
          </div>
        </div>
      </motion.div>
